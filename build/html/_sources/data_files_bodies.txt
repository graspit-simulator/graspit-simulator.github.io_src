...
dynamic body adds to that definition.
...
-  an ``.xml`` file that contains all the GraspIt! specific information,
...
(``$GRASPIT/models/objects/flask.xml``):
...
The format is fully XML adherent, you should be able to also read it in
with any XML parser (such as your web browser). This is the list of tags
that GraspIt! will look for in the Body file:

-  ``<geometryFile>`` - this is the only mandatory tag. It contains the
...
-  ``<material>`` - can be any of the materials used in GraspIt!. If it
...
-  ``<youngs>`` - the Young’s modulus of the object, in MPa. This can be
