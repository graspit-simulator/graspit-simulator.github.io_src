...
scanner, object location from a tracker, robot pose, etc.
...
In our work, we have connected GraspIt! to the following external
devices:

-  a real Barrett hand

-  a Flock of Birds tracker that can be used to move objects or robots
...
each of the three pieces of hardware mentioned above.
...
A Flock of Birds tracker can be used to set the position of any element
(body or robot) in the GraspIt! simulation world. The following steps
must be followed:

-  at load time, the Robot configuration file, or the Body file, must
...
-  using the GraspIt! GUI, you must turn on Flock of Birds tracking.
...
A Cyberglove can be used to set the pose of a hand. However, hand models
...
-  at load time, the Robot configuration file must indicate the name of
   the Cyberglove calibration file that is to be used. See the
   ``HumanHand20DOF`` config file for an example.

-  using the GraspIt! GUI, you must turn on Cyberglove tracking. When
...
A calibration file is provided with the ``HumanHand20DOF`` model. We
