...
A GraspIt! DOF is responsible for telling us how all of the joints that
...
its links), what happens to the other joints connected to the same DOF?

In dynamics mode, a DOF is also responsible for enforcing the same mode
...
classes described below for details.
...
version of the HumanHand). See the ``RigidDOF`` class for details.
